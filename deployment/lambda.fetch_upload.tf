...
resource "null_resource" "build_fetch_upload" {
  triggers = {
    handler      = base64sha256(file("${path.module}/../src/fetch_upload/handler.py"))       # TODO: change here
    requirements = base64sha256(file("${path.module}/../src/fetch_upload/requirements.txt")) # TODO: change here
    build        = base64sha256(file("${path.module}/../src/fetch_upload/build.sh"))         # TODO: change here
  }

  provisioner "local-exec" {
    command = "bash ${path.module}/../src/fetch_upload/build.sh"
  }
}


# Tells Terraform to compress your source code with dependencies
data "archive_file" "fetch_upload" {
  type        = "zip"
  output_path = "${path.module}/../src/fetch_upload.zip" # TODO: change here
  source_dir  = "${path.module}/../src/fetch_upload"     # TODO: change here

  depends_on = [
    null_resource.build_fetch_upload # TODO: change here
  ]
}
...
resource "aws_lambda_function" "fetch_upload" {
  filename      = data.archive_file.fetch_upload.output_path
  function_name = "${var.group_name}_${terraform.workspace}_fetch_upload" # TODO: change here
  ...
  source_code_hash = data.archive_file.fetch_upload.output_base64sha256 # TODO: change here
...
resource "aws_lambda_permission" "fetch_upload" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.fetch_upload.function_name # TODO: change here
  principal     = "apigateway.amazonaws.com"

  source_arn = "${data.aws_apigatewayv2_api.api_gateway_global.execution_arn}/*/*/fetch_upload" # TODO: change here
}
...
resource "aws_apigatewayv2_integration" "fetch_upload" {
  api_id           = var.gateway_api_id
  integration_type = "AWS_PROXY"

  integration_uri    = aws_lambda_function.fetch_upload.invoke_arn # TODO: change here
...
resource "aws_apigatewayv2_route" "fetch_upload" {
  api_id    = var.gateway_api_id
  route_key = "POST /${var.group_name}/fetch_upload" # TODO: change here

  target = "integrations/${aws_apigatewayv2_integration.fetch_upload.id}" # TODO: change here
...
resource "aws_cloudwatch_log_group" "fetch_upload_log" {
  name              = "/aws/lambda/${aws_lambda_function.fetch_upload.function_name}" # TODO: change here
  retention_in_days = 7
  lifecycle {
