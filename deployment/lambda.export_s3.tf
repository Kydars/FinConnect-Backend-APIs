...
resource "null_resource" "build_export_s3" {
  triggers = {
    handler      = base64sha256(file("${path.module}/../src/export_s3/handler.py"))       # TODO: change here
    requirements = base64sha256(file("${path.module}/../src/export_s3/requirements.txt")) # TODO: change here
    build        = base64sha256(file("${path.module}/../src/export_s3/build.sh"))         # TODO: change here
  }

  provisioner "local-exec" {
    command = "bash ${path.module}/../src/export_s3/build.sh"
  }
}


# Tells Terraform to compress your source code with dependencies
data "archive_file" "export_s3" {
  type        = "zip"
  output_path = "${path.module}/../src/export_s3.zip" # TODO: change here
  source_dir  = "${path.module}/../src/export_s3"     # TODO: change here

  depends_on = [
    null_resource.build_export_s3 # TODO: change here
  ]
}
...
resource "aws_lambda_function" "export_s3" {
  filename      = data.archive_file.export_s3.output_path
  function_name = "${var.group_name}_${terraform.workspace}_export_s3" # TODO: change here
  ...
  source_code_hash = data.archive_file.export_s3.output_base64sha256 # TODO: change here
...
resource "aws_lambda_permission" "export_s3" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.export_s3.function_name # TODO: change here
  principal     = "apigateway.amazonaws.com"

  source_arn = "${data.aws_apigatewayv2_api.api_gateway_global.execution_arn}/*/*/export_s3" # TODO: change here
}
...
resource "aws_apigatewayv2_integration" "export_s3" {
  api_id           = var.gateway_api_id
  integration_type = "AWS_PROXY"

  integration_uri    = aws_lambda_function.export_s3.invoke_arn # TODO: change here
...
resource "aws_apigatewayv2_route" "export_s3" {
  api_id    = var.gateway_api_id
  route_key = "POST /${var.group_name}/export_s3" # TODO: change here

  target = "integrations/${aws_apigatewayv2_integration.export_s3.id}" # TODO: change here
...
resource "aws_cloudwatch_log_group" "export_s3_log" {
  name              = "/aws/lambda/${aws_lambda_function.export_s3.function_name}" # TODO: change here
  retention_in_days = 7
  lifecycle {
